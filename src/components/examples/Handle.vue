<template>
	<div class="drop-zone">
		...
		>
			<h2>{{ item.title }}</h2>
			<p>Description</p>
			<svg aria-hidden="true" height="32" viewBox="0 0 16 16" width="32" fill="currentColor" @mousedown="mouseStart($event)" @mouseleave="mouseLeave($event)">
...
const onOver = (event) => {
	const dragEl = findDragEl(event.target)
	if (dragEl) {
		dragEl.style.border = "1px solid #33a06f"
	}
}
...
const onLeave = (event) => {
	const dragEl = findDragEl(event.target)
	if (dragEl) {
		dragEl.style.border = "1px solid #363636"
	}
}
